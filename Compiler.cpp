...
#include "Parser.h" // 需要包含解析器来生成 AST
...
// 访问赋值语句: ma = ...
void Compiler::visit(AssignmentStmt& stmt) {
...
void Compiler::visit(CallExpr& expr) {
    ...
    if (auto* var = dynamic_cast<VariableExpr*>(expr.callee.get())) {
        ...
        if (funcName == "plot") {
            emitByteWithOperand(OpCode::CALL_PLOT, expr.arguments.size());
        } else {
...
// Corrected Compiler::visit(BinaryExpr& expr)
...
void Compiler::emitByteWithOperand(OpCode op, int operand) {
...
// 解析变量并发出 LOAD 指令
void Compiler::resolveAndEmitLoad(const Token& name) {
    // 检查是内置变量还是全局变量
...
// 解析变量并发出 STORE 指令
void Compiler::resolveAndEmitStore(const Token& name) {
    if (globalVarSlots.find(name.lexeme) == globalVarSlots.end()) {
        globalVarSlots[name.lexeme] = nextSlot++;
    }
